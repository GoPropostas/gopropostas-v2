...
def render_topbar(imob=None):
    titulo = imob["nome"] if imob else "GoPropostas"
    subtitulo = "Sistema corporativo de propostas imobiliárias"
...
def excel_para_pdf(arquivo):
    ...
    if not executavel:
        raise FileNotFoundError(
            "LibreOffice não encontrado. Verifique se o LibreOffice está instalado em "
            r"'C:\Program Files\LibreOffice\program\soffice.exe' "
            "ou ajuste o caminho na função excel_para_pdf."
        )

    pasta_saida = os.path.dirname(os.path.abspath(arquivo))
    ...
    if not os.path.exists(caminho_pdf):
        raise FileNotFoundError(f"PDF não foi gerado: {caminho_pdf}")
        ...
        if st.button("Criar conta", key="btn_cadastro", use_container_width=True):
            ...
            try:
                ...
                if resp.status_code in (200, 201):
                    ...
                    try:
                        st.error(f"Erro ao criar conta: {resp.json()}")
                    except Exception:
...
def painel_aprovacoes():
    ...
    solicitacoes = listar_solicitacoes_pendentes() if pode_ver_todas() else listar_solicitacoes_pendentes_por_imobiliaria(st.session_state["imobiliaria_id"])
...
def tela_nova_proposta():
    ...
    with col_form_1:
        ...
        data_nascimento = st.date_input("Data de nascimento do cliente", value=date(1980, 1, 1), min_value=date(1900, 1, 1), max_value=date.today(), key="data_nascimento")
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if valor_cliente < valor_minimo_entrada:
        erros_validacao.append(f"Entrada cliente menor que o mínimo. Mínimo recomendado: {formatar_moeda(valor_minimo_entrada)}")
    ...
    if abs(valor_total_distribuicao - valor_total_comissao) > 0.01:
        erros_validacao.append(f"Contrato de intermediação inválido: a soma da distribuição deve ser exatamente {pct_total:.2f}% do valor total do negócio.")
    ...
    if entrada_quitada:
        ...
        if usar_diferente:
            st.info(f"{parcelas_iguais}x de {formatar_moeda(valor_parcela_igual)} + 1x de {formatar_moeda(parcela_diferente)}")
        else:
...
menu = st.sidebar.radio(
    "📂 Navegação",
    ["Início", "Solicitações", "Membros", "Empreendimentos", "Regras", "Nova Proposta"],
)
